...
namespace ssao {
  ...
  void SsaoPass::render(CommandBuffer &cmdBuf, const Camera &camera) {
    ...
    cmdBuf.pushFragmentUniformRaw(0, KERNEL_SAMPLES.data(),
                                  SAMPLES_PAYLOAD_BYTES);
    cmdBuf.pushFragmentUniformRaw(1, &proj, sizeof(proj));
    ...
    for (size_t i = 0; i < 2; i++) {
      ...
      cmdBuf.pushFragmentUniformRaw(0, &blurDir, sizeof(blurDir));
      rend::bindFragmentSamplers(
          render_pass, 0,
          {{
